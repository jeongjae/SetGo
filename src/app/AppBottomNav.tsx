...
export function AppBottomNav({ activeView, onNavigate }: AppBottomNavProps) {
  ...
  return (
    <nav aria-label="Primary navigation" className="app-bottom-nav mx-auto grid w-full max-w-md shrink-0 grid-cols-5 border-t border-sg-border bg-[rgba(255,255,255,0.9)] px-1.5 pt-1.5 text-sg-label shadow-[0_-8px_24px_rgba(0,0,0,0.04)] backdrop-blur-xl">
        ...
        return (
          <button
            ...
            className={`relative flex min-h-14 flex-col items-center justify-center gap-1 rounded-2xl text-[10px] font-bold transition-all active:scale-95 ${
              active ? 'text-sg-action' : 'text-sg-tertiary-label hover:bg-sg-fill hover:text-sg-label'
            }`}
          >
            <Icon aria-hidden="true" size={21} className={active ? 'text-sg-action' : 'text-sg-tertiary-label'} />
